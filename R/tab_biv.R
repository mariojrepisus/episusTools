...
tab_biv <- function(banco, expos, desf, valores) {
  ...
  cat(
    "\n\n####   Analise bivariada com varias combinacoes   ####\n\nAnalisando",
    length(expos), "exposicoes e", length(desf), "desfechos\n\n"
  )
  ...
  for (i in seq_along(expos)) {
    ...
    for (des in desf) {
      data <- banco |>
        ...
        dplyr::mutate(
          !!exp := factor(.data[[exp]], labels = valores, levels = valores),
          !!des := factor(.data[[des]], labels = valores, levels = valores)
        )

      res  <- epitools::epitab(data[[exp]], data[[des]],
